 ...
 * This helps handle cold-start 503 errors and CORS errors from Render's free tier.
 */
const fetchWithRetry = async (url: string, options?: RequestInit, retries = 5, delay = 1000): Promise<Response> => {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      // If it's a 503 error, it might be a cold start, so we retry.
      if (res.status !== 503 && res.status >= 400 && res.status < 500) {
        return res; // Don't retry client errors (except maybe 429, but let's keep it simple)
      }
    } catch (err) {
      // TypeError is thrown for CORS errors (which happen during 503 from load balancer)
...
export const getPlayerRegistrations = async (): Promise<PlayerRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/players`);
...
export const deletePlayerRegistration = async (id: string): Promise<PlayerRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/players/${id}`, { method: 'DELETE' });
...
export const updatePlayerRegistration = async (id: string, data: Partial<PlayerRegistration>): Promise<PlayerRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/players/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export const updatePlayerStatus = async (id: string, status: PlayerRegistration['status']): Promise<PlayerRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/players/${id}/status`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export const getTournamentApplications = async (): Promise<TournamentApplication[]> => {
  const res = await fetchWithRetry(`${API_BASE}/tournaments`);
...
export const deleteTournamentApplication = async (id: string): Promise<TournamentApplication[]> => {
  const res = await fetchWithRetry(`${API_BASE}/tournaments/${id}`, { method: 'DELETE' });
...
export const updateTournamentStatus = async (id: string, status: TournamentApplication['status']): Promise<TournamentApplication[]> => {
  const res = await fetchWithRetry(`${API_BASE}/tournaments/${id}/status`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ status })
  });
...
export const getSponsorRegistrations = async (): Promise<SponsorRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/sponsors`);
...
export const deleteSponsorRegistration = async (id: string): Promise<SponsorRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/sponsors/${id}`, { method: 'DELETE' });
...
export const updateSponsorStatus = async (id: string, status: SponsorRegistration['status']): Promise<SponsorRegistration[]> => {
  const res = await fetchWithRetry(`${API_BASE}/sponsors/${id}/status`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ status })
  });
...
export const resetToSeedData = async (): Promise<boolean> => {
  const res = await fetchWithRetry(`${API_BASE}/db/reset`, { method: 'POST' });
  return res.ok;
};

export const clearAllData = async (): Promise<boolean> => {
  const res = await fetchWithRetry(`${API_BASE}/db/clear`, { method: 'POST' });
  return res.ok;
};
...
export const updateScheduledGame = async (id: string, game: Omit<ScheduledGame, 'id' | 'createdAt'>): Promise<ScheduledGame | null> => {
  const res = await fetchWithRetry(`${API_BASE}/games/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(game)
  });
...
export const deleteScheduledGame = async (id: string): Promise<boolean> => {
  const res = await fetchWithRetry(`${API_BASE}/games/${id}`, { method: 'DELETE' });
...
export const getGameFees = async (): Promise<GameFee[]> => {
  const res = await fetchWithRetry(`${API_BASE}/game-fees`);
...
export const updateGameFee = async (id: string, fee?: string, isPublished?: boolean): Promise<GameFee[]> => {
  const res = await fetchWithRetry(`${API_BASE}/game-fees/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export const getPaymentConfig = async (): Promise<PaymentConfig> => {
  const res = await fetchWithRetry(`${API_BASE}/payments/config`);
